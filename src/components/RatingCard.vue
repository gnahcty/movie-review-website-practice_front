  ...
  <div class="flex-center column q-gutter-y-sm" style="min-width: 320px;">
    ...
    <div class="flex flex-center q-gutter-x-sm">
      <q-btn flat round icon="fa-regular fa-eye" size="xl" :color="userReview.watched ? 'green' : 'grey'"
        :disable="!user.isLogin" @click="seen()" />
      <q-btn flat round :color="userReview.like ? 'red' : 'grey'" icon="favorite" @click="like()" :disable="!user.isLogin"
        size="xl" />
      <q-btn flat round :color="inWatchList ? 'blue' : 'grey'" icon="more_time" @click="addToWatchList()"
  ...
  <q-dialog v-model="reviewDialog" full-width>
    <div id="reviewDialog" class="border10 column q-gutter-xl bg-white">
      ...
      <div class="row flex-center q-gutter-md">
        <q-btn flat label="submit" class="border5 rounded15 lilita" @click="submitReview" :disable="editorEmpty" />
...
const inWatchList = computed(() => {
  return user.watchList.some(movie => movie.id === props.film.id.toString())
}
)
...
const checkWatchList = async () => {
  ...
  user.watchList = data.result.watchList
}
